...
2. **Universal Standard Aggregate & Structure Initialization:** All data structures, descriptors, configuration records, and aggregate types originating from operating system, platform, or external SDK interfaces must be initialized using the target language standard's native designated structural initializers and direct compile-time value bindings, completely eliminating legacy factory helpers, procedural setup routines, and raw byte-clearing memory operations.
...
2. **Direct Designated Aggregate Initialization Over Wrapper Convenience:** Whenever a platform data structure, descriptor, transform matrix, or configuration record can be initialized directly via the target language standard's native compile-time designated structural initializers or aggregate bindings, the agent must initialize the underlying structure directly, bypassing all runtime convenience multipliers, wrapper classes, fluent builder helpers, and procedural constructor functions. Reasoning that a runtime wrapper helper is acceptable because it performs convenience arithmetic or bundles multiple initialization steps is prohibited whenever declarative aggregate initialization is supported.
3. **Negative-Inference Atomic Token Verification:** The agent must conduct an explicit, token-level audit across every statement, hunting specifically for:
   - Platform function calls that exist in legacy vs modern architecture-safe pairs, ensuring the modern pointer-width-safe variant is used exclusively across all call sites.
   - External convenience wrappers, arithmetic helper wrappers, and procedural transform adapters that obscure direct structural aggregate initialization.
4. **Clean Build Rejection as Proof:** A clean build, zero compiler warnings, and successful runtime execution are baseline technical requirements, never evidence that architecture-width correctness or modern aggregate initialization standards have been achieved.
...
*Failure class: the agent inspects an API call, notes that it compiles cleanly with zero warnings under strict compiler flags and looks syntactically standard, but fails to realize the call utilizes a legacy truncated platform variant or an unneeded runtime wrapper helper where direct compile-time designated aggregate initialization is required.*
