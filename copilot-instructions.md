...
3. If the fix retires, replaces, disables, downgrades, bypasses, rebuilds, or writes different data into any validation control, security control, hook, stub, pointer, handle, buffer, cache, allocation, or other shared state — including changing which code path populates a shared data structure, or changing the contents that flow into one — explicitly name:
   - who creates it
...
"Authoritative documentation" does not include the agent's training data, memorized API references, or any knowledge source that cannot be opened and read as a file — only material that exists as a readable file in the workspace or at a URL the agent can fetch counts.
...
"Shared data structure" is not limited to code-level tracking containers (maps, sets, registries, flags) that the project maintains to manage state — it includes any runtime object that the function modifies and that other code can observe, regardless of whether the observation goes through the project's own tracking or through the external system's own resolution mechanisms. When a function writes into an external-system object, that object is a shared data structure even if the project does not track its consumers in an explicit container. The external system's own traversal mechanisms — parent-child inheritance chains, fallback resolution, virtual dispatch, observer notifications — create consumers of that object that will not appear when searching for the project's tracking container names. A search that finds only consumers of the tracking containers has found a subset, not the full set.

1. Identify every shared data structure whose contents, population, or update timing changes as a result of the behavioral change — not only structures the function explicitly names, but also structures reachable through pointers, parent chains, or inheritance hierarchies that the function's output flows into. This identification must be performed by tracing each write and data-flow output of the changed function in the code — not by reasoning about the function's purpose from within the function's body alone. For each value the function writes, passes to another function, or stores in a field, trace where that value ends up by reading the receiving code. If the destination is accessible to other code paths, it is a shared data structure and steps 2-5 apply to it. A conclusion of "no shared structures are affected" is only valid when every write and data-flow output has been traced to its destination in code — reaching that conclusion without performing this trace is a protocol violation. The identified shared structures — or the explicit trace showing none exist — must be enumerated by name in the response; an identification that stays internal is indistinguishable from one that was never performed.
...
5. **Validate every exclusion.** When the changed code skips, excludes, or structurally omits a category of entity from processing, verify that the omitted entities are genuinely unaffected by the full effect chain the code participates in — not just by the immediate action of the changed code. An exclusion is any mechanism — explicit or structural — by which entities that could be affected by the operation's downstream results are not processed. Explicit exclusions include `continue`, early return, conditional guard, and filter predicate. Structural exclusions include iterating only over entries in a tracking map that by construction omits entities affected through paths the map does not track, selecting data from a source that does not represent the full set of affected entities, or scoping processing to a container smaller than the full affected set. Structural exclusions are the highest-risk category — they have no visible skip condition in the code and are undetectable unless the agent compares the code's processing scope against the full set of entities affected through all paths, including the external system's own resolution mechanisms.

   "The operation" is the full effect chain the code participates in — from the initial trigger through every intermediate step to the final observable outcome — not the immediate action of the changed function alone. A function that populates a tracking set is part of a larger operation; the question is whether omitted entities are affected by that larger operation, not whether they need to be in the tracking set. Before evaluating whether any exclusion is safe, identify the external system's traversal and resolution mechanisms for the domain the code operates in — by reading reference material (decompilation, reversed source, documentation) where available, not by reasoning from the project's code alone. An exclusion cannot be validated against external-system paths that have not first been identified. If reference material might exist but has not been found, search the workspace before concluding it is absent (same obligation as "When analysis or fixes involve external system behavior" in Rule 2). "Unaffected" means no path exists — in the project's own logic or in the external system's behavior — through which the omitted entities can observe, inherit, or be altered by the operation's results.

   Each exclusion — explicit or structural — must be named in the response with: (a) what entities are omitted and by what mechanism, (b) what external-system traversal mechanisms were identified for this domain, (c) whether any identified mechanism can deliver the operation's effects to the omitted entities, and (d) the conclusion. An exclusion reported without naming the external-system mechanisms it was checked against is unvalidated. An exclusion validated only against the code's own tracking structures is unvalidated — the tracking scope is typically narrower than the effect scope. When an exclusion is found to be incomplete — where omitted entities are affected through paths the code does not track — the exclusion must be fixed or the affected entities must be accounted for before the change is marked complete.

The stopping condition is: every consumer of every affected shared data structure has been read and confirmed compatible with the new contents, and every exclusion (explicit or structural) has been validated against the full effect chain including external-system resolution mechanisms. A consumer that was compatible with the old contents is not presumed compatible with the new ones — compatibility must be confirmed by reading, not by reasoning from the change alone.
...
7. **When the gate sits in a shared transport, dispatch, queue, or send path, build the exception matrix from callers, not from the local function's apparent purpose.** A generic transport function often carries both pre-ready control traffic and normal post-ready traffic. Before changing the gate, enumerate every caller and classify each one by exact discriminator set: packet type, message type, sub-id, opcode, callback identity, and lifecycle state where applicable. Do not permit a broad category through the gate if only a narrower subset is required. If the narrowed subset has not been derived from actual callers read in code, the gate change is incomplete.
8. **Do not use parsed metadata side effects as the authority for lifecycle gates when the system has a stronger state boundary.** Parsed build numbers, version strings, formatted text fields, compatibility banners, or other derived metadata may still be useful for logging or legacy feature selection, but they must not become the authoritative gate for readiness, transport eligibility, or timeout phase changes if the codebase has a more direct completion signal. If a stronger state boundary exists, use it. If it does not, state that gap explicitly before proceeding.
