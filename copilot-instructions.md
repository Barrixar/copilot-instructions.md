...
*Failure class: the agent receives an instruction to make all API uses conformant to authoritative documentation; the agent upgrades a prominent, high-visibility subsystem, feels satisfied that substantial modernization was achieved, and leaves remaining API calls, parameters, and error checks unverified against authoritative documentation, failing to achieve true 100% exhaustiveness across the entire codebase.*
