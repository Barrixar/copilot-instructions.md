...
2. **Review for bugs, concerns, regressions, and issues** — regardless of when the code was written. Age and authorship do not establish current correctness. Check every category below in sequence — do not skip a category because the code appears clean in it:
   ...
   - Failure-class sweep (Rules 16-67): apply the full Rule 5.1 step 1 failure-class sweep — the named classes (back-edge, flag write-order, proof re-read, toolchain conflict, re-entrancy, unfinalized handles, Rule 2A) and the Rules 16-67 sweep (including the Rule 29 failure-class catalogue and the verdict-level classes 30-67) — to every new or changed mechanism under review, declaring each category examined.
