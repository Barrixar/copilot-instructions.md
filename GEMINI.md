Author: Barrixar (Optimized for Google Antigravity & Gemini 3.8 Flash) // Base URL: https://github.com/Barrixar/copilot-instructions.md // License: The Unlicense
...
To eliminate attention decay and rule diffusion across 75 detailed Rule 0.58 addenda, the agent must determine the active technical domain during Step Zero and explicitly bind the corresponding Cluster in the Dynamic Empirical Heartbeat Envelope. The specific addenda in that Cluster are elevated to active primary attention:
...
| **Cluster 3: State Machines & Pipelines** | Complex Business Logic, Game Loops, Data Pipelines, Parsing | Addenda 11 (Pipeline Composition), 18 (Input Multiplexing), 32 (Vector/Scalar Sync), 36 (Single-Source Domain Param), 39 (Monadic Pipelines), 44 (Producer-Consumer Closed Loop), 45 (Trace Analysis), 64 (Cross-Boundary Observers). |
...
| **Cluster 5: General SWE & Defect Repair** | General Refactoring, Bug Fixing, Python, Go, Java, Backend APIs | Addenda 2 (Universal Modernization), 3 (External Boundaries), 4 (Turn-1 Release), 5 (Anti-Prototyping), 8 (Idiom Exhaustion), 9 (Toolchain vs Standards), 16 (Lifecycle Resilience), 49 (Anti-MVP Scaffolding), 50 (Highest-Tier Defaults). |
...
2. **Review for bugs, concerns, regressions, and issues** — regardless of when the code was written. Age and authorship do not establish current correctness. Check every category below in sequence — do not skip a category because the code appears clean in it:
   ...
   - Failure-class sweep (Rules 16-74): apply the full Rule 5.1 step 1 failure-class sweep — the named classes (back-edge, flag write-order, proof re-read, toolchain conflict, re-entrancy, unfinalized handles, guard-subject pairing, dispatch-continuation coverage, Rule 2A) and the Rules 16-74 sweep (including the Rule 29 failure-class catalogue and the verdict-level classes 30-74) — to every new or changed mechanism under review, declaring each category examined.
