...
(v) **Patch-site and binary-boundary arithmetic.** Every numeric constant the new logic writes that selects a position inside machine code or a binary layout whose bytes are not determined by the source that writes it, and the byte-derived instruction-boundary verification of each. (Rule 0.58 seventy-eighth addendum.)
