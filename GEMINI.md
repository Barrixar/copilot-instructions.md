...
5. For multi-file refactors, features, architectural modifications, or non-trivial bug fixes, render the written Pre-Write Requirement Matrix (Rule 0.58) in visible output or native `implementation_plan.md` before issuing any write or edit tool call. For trivial atomic edits (typos, single config values), use the Streamlined Fast-Path (Rule 0.58 Item 4).
...
When beginning task processing, write the exact words **"Antigravity Quality Discipline active. Looking for triggers."** at the start of your thinking/reasoning and reply, before any other analysis. This serves as the verifiable heartbeat that the ruleset is actively controlling execution.

Writing any heartbeat phrase without actively adhering to the underlying protocol is fabrication — the most serious violation in this document. The agent must never claim adherence without executing the required checks.
...
This rule establishes five high-leverage architectural invariants capitalizing on Gemini 3.8 Flash's dedicated High thinking capacity and Antigravity's deep OS and tool integration, specifically engineered to eliminate regressions, prevent code drift, and guarantee mathematical correctness:

### 1. The Pre-Act Blast-Radius Thinking Protocol (High Thinking Allocation)
...
### 2. Pre-Edit Reproduction Test Invariant (Empirical TDD for Defect Resolution)
...
### 3. Strict Diff Minimality & The Anti-Scope-Inflation Invariant
...
### 4. Compulsory Fast Typecheck & Linter Execution Gate via `run_command`
...
### 5. Harmonization of Rule 0.58 with Native Planning Artifacts
...
- **Fast-Path Remains in Chat:** Trivial atomic edits continue to use the Streamlined Fast-Path directly in the visible chat body as defined in Rule 0.58.
...
When beginning this scan, write the exact words **"Antigravity Quality Discipline active. Looking for triggers."** at the start of your thinking/reasoning and reply, before any other analysis. This is mandatory. It serves as a verifiable heartbeat that the Antigravity Quality Discipline is active and Step zero was actually performed rather than skipped. A response where this phrase does not appear at the start of the reasoning is invalid and must not be sent — for the same reason a trigger-matched response is invalid without its protocol: the check cannot be proven to have occurred.
...
If the visible response does not begin with **"Antigravity Quality Discipline active. Looking for triggers."** then the task-start protocol has failed and must be restarted. The sequential dependency is defined here and in the closing gate at the end of this file.
...
4. **Render the Pre-Write Requirement Matrix before authoring code (Scoped Gate & Planning Artifacts).**
   ...
   - **Streamlined Fast-Path for Trivial Edits:** For trivial atomic changes (strictly limited to: fixing a single typo in text/comments, updating a port/version number, adjusting an isolated constant, or simple cosmetic formatting where the blast radius is provably zero), the agent is exempt from rendering the multi-column Markdown table. Instead, it must satisfy the fast-path pre-write gate: state the exact verbatim constraint and the target line in a single visible sentence before issuing `replace_file_content`.
...
4. **No Loophole for Minor or Localized Edits (Anti-Satisficing Invariant):** Anti-satisficing applies unconditionally across all tasks producing code, configuration, or documentation changes. An agent may not self-exempt from full production standard adherence, type safety, boundary hardening, or negative inference scanning on the basis that a change is "small", "a patch", "a simple fix", or "an incremental step". While trivial atomic edits may use the Streamlined Fast-Path for the pre-write matrix format (as defined in Rule 0.58), the emitted code must still be 100% production-hardened from Line 1.
...
3. **Pre-Write Gate Enforcement:** Any comprehensive file creation, write, or non-trivial modification executed without a prior visible Pre-Write Requirement Matrix (or without the native `implementation_plan.md` artifact, or the explicit Streamlined Fast-Path declaration for trivial atomic edits, as defined in Rule 0.58) is an invalid operation that must be aborted and restarted.
...
1. **"Antigravity Quality Discipline active. Looking for triggers."** (Task start heartbeat)
2. [Task execution: VCS diff via `run_command`, Pre-Write Matrix (visible or `implementation_plan.md`) or Fast-Path, Blast-Radius reasoning & Reproduction test (Rule 0.10), surgical edits via `replace_file_content`, Compulsory Typecheck Gate via `run_command`]
...
4. Rule 5.1 report (Written requirement ledger + zero-finding re-pass)
