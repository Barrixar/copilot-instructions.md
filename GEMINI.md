...
|---|---|---|
| **Cluster 1: Systems & Native ABI** | C, C++, Rust, Zig, OS APIs, Direct Memory, Drivers, C-ABIs | Addenda 1 (Line-1 RAII), 19 (Zero Alloc Loop), 23 (DPI/OS), 31 (Async Teardown/UAF), 42 (Zero Raw Copy), 43 (Buffer Pinning), 60 (Smart Handle Out-Pointers), 62 (Aggregate Struct Init). |
| **Cluster 2: Concurrency & Async** | Workers, Threads, Message Pumps, Event Loops, Asynchronous I/O | Addenda 5 (Modular Architecture), 10 (State Integration), 21 (State Machine Transitions), 22 (Message Loop/Pump), 35 (Queue Validation), 48 (Teardown Lifecycles), 57 (HW Queue Lifecycles). |
...
| **Cluster 5: General SWE & Defect Repair** | General Refactoring, Bug Fixing, Python, Go, Java, Backend APIs | Addenda 2 (Universal Modernization), 3 (External Boundaries), 4 (Turn-1 Release), 5 (Anti-Prototyping), 8 (Idiom Exhaustion), 9 (Toolchain vs Standards), 16 (Lifecycle Resilience), 49 (Anti-MVP Scaffolding), 50 (Highest-Tier Defaults), 77 (Effect-Class Entry Exhaustion). |
...
2. **Review for bugs, concerns, regressions, and issues** — regardless of when the code was written. Age and authorship do not establish current correctness. Check every category below in sequence — do not skip a category because the code appears clean in it:
   ...
   - Failure-class sweep (Rules 16-74): apply the full Rule 5.1 step 1 failure-class sweep — the named classes (back-edge, flag write-order, proof re-read, toolchain conflict, re-entrancy, unfinalized handles, guard-subject pairing, dispatch-continuation coverage, derived-representation drift, effect-class channel gap, Rule 2A) and the Rules 16-74 sweep (including the Rule 29 failure-class catalogue and the verdict-level classes 30-74) — to every new or changed mechanism under review, declaring each category examined.
