...
To eliminate attention decay and rule diffusion across 77 detailed Rule 0.58 addenda, the agent must determine the active technical domain during Step Zero and explicitly bind the corresponding Cluster in the Dynamic Empirical Heartbeat Envelope. The specific addenda in that Cluster are elevated to active primary attention:

| Domain Cluster | Target Technologies & Scope | Active Primary Invariants (Rule 0.58 Addenda) |
|---|---|---|
| **Cluster 1: Systems & Native ABI** | C, C++, Rust, Zig, OS APIs, Direct Memory, Drivers, C-ABIs | Addenda 1 (Line-1 RAII), 19 (Zero Alloc Loop), 23 (DPI/OS), 31 (Async Teardown/UAF), 42 (Zero Raw Copy), 43 (Buffer Pinning), 60 (Smart Handle Out-Pointers), 62 (Aggregate Struct Init), 78 (Patch-Site Boundaries), 79 (Scan-Candidate Validation), 81 (Foreign-Binary Channels), 82 (Pre-Write Byte Derivation), 83 (Guard-Pattern Completeness), 84 (Non-Activation Signals), 85 (Optional-Degradation Preservation), 86 (Derived-Verification Re-Derivability). |
...
| **Cluster 5: General SWE & Defect Repair** | General Refactoring, Bug Fixing, Python, Go, Java, Backend APIs | Addenda 2 (Universal Modernization), 3 (External Boundaries), 4 (Turn-1 Release), 5 (Anti-Prototyping), 8 (Idiom Exhaustion), 9 (Toolchain vs Standards), 16 (Lifecycle Resilience), 49 (Anti-MVP Scaffolding), 50 (Highest-Tier Defaults), 77 (Effect-Class Entry Exhaustion), 79 (Scan-Candidate Validation), 80 (Last-Event Freshness), 81 (Foreign-Binary Channels). |
